        ...
        for link in network_data['config']:
            ...
            if link.get('type'):
                ...
                if link['type'] == 'vlan':
                    ...
                    parsed_link['type'] = link['type']
                    bond_info = parsed_link['bond_info']
                    if link.get('bond_interfaces'):
                        bond_info['bond_members'] = link['bond_interfaces']
                    if link.get('params').get('bond-mode'):
                        bond_info['bond_mode'] = \
    ...
    def _parse_l3_network_data(self, network_data, network_l2_config):
        ...
        for network_config in network_data['config']:
            ...
            for subnet in network_config.get('subnets'):
                ...
                parsed_network["id"] = network_config.get('id')
