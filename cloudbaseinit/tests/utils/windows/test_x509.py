...
class CryptoAPICertManagerTests(unittest.TestCase):
    ...
    def _test_create_self_signed_cert(self, mock_CertFreeCertificateContext,
                                      ...
                                      mock_malloc, mock_free, mock_get_certstr,
    ...
    @mock.patch('cloudbaseinit.utils.windows.cryptoapi.'
                'CryptBinaryToString')
    def _test_get_cert_str(self, mock_binary_to_string, ret_value):
        mock_binary_to_string.side_effect = ret_value
        mock_DWORD = self._ctypes.wintypes.DWORD
        mock_cert_context_p = mock.Mock()
        mock_cert_context_p.contents = mock.Mock()
        mock_byref = self._ctypes.byref

        if all(ret_value):
            result = self._x509_manager._get_cert_str(mock_cert_context_p)
            self.assertEqual(
                result, self._ctypes.create_unicode_buffer.return_value.value)
        else:
            with self.assertRaises(self.x509.cryptoapi.CryptoAPIException):
                self._x509_manager._get_cert_str(mock_cert_context_p)
        mock_DWORD.assert_called_with(0)

        expected = [
            mock.call(
                mock_cert_context_p.contents.pbCertEncoded,
                mock_cert_context_p.contents.cbCertEncoded,
                self.x509.cryptoapi.CRYPT_STRING_BASE64,
                None, mock_byref(mock_DWORD.return_value))
        ]
        ...
        if len(ret_value) > 1:
            expected += [
                mock.call(
                    ...
                    mock_cert_context_p.contents.cbCertEncoded,
                    self.x509.cryptoapi.CRYPT_STRING_BASE64,
                    self._ctypes.create_unicode_buffer(
                        mock_DWORD.return_value.value),
                    mock_byref(mock_DWORD.return_value))
            ]

        self.assertEqual(
            expected,
            mock_binary_to_string.call_args_list)

    def test_get_cert_str_CryptBinaryToString_fails(self):
        self._test_get_cert_str(ret_value=[False])

    def test_get_cert_str_CryptBinaryToString_fails_second(self):
        self._test_get_cert_str(ret_value=[True, False])

    def test_get_cert_str(self):
        self._test_get_cert_str(ret_value=[True, True])
